...
class StEasyContentClass extends ObjectModel
{
	...
	public static $definition = array(
		...
		'fields'    => array(
            ...
            'id_product'      => array('type' => self::TYPE_STRING, 'size' => 4096),
            ...
            'bo_tab'                  => array('type' => self::TYPE_INT, 'validate' => 'isBool'),
	...
	public static function getListContent($identify=0, $type=0, $active=0, $total = false)
	{
        $page = Tools::getValue('submitFiltersteasycontent', 1);
        $page_size = Tools::getValue('steasycontent_pagination', 50);
        $page <= 0 && $page = 1;
        $page_size <= 0 && $page_size = 50;
        $limit = (($page - 1) * $page_size ) . ', ' . $page_size;
	    $id_lang = Context::getContext()->language->id;
        Shop::addTableAssociation('st_easy_content', array('type' => 'shop'));
        $where = $type  && $type != 100 ? ' AND sec.`type`='.(int)$type : '';
        $search_location = Tools::getValue('steasycontentFilter_location');
        $search_location && $where .= ' AND secl.`name` like "%'.$search_location.'%"';
        ...
        switch($type) {
            ...
            case 2:
                ...
                break;
            case 3:
                $where .= ' AND (sec.`id_cms` = 0 OR sec.`id_cms` = '.(int)Tools::getValue('id_cms').')';
                break;
            case 4:
            case 5:
            case 23:
            case 25:
                $where .= ' AND (sec.id_category = 0 AND sec.id_manufacturer = 0 OR (sec.id_category > 0 AND sec.id_category = '.(int)Tools::getValue('id_category').'))';
                break;
            case 6:
            case 7:
            case 8:
            ...
            case 26:
            case 27:
                $where .= ' AND ((sec.id_manufacturer = 0 AND sec.id_category = 0 AND sec.id_product = "")';
	            $product = new Product((int)Tools::getValue('id_product'));
	            if ($product->id_manufacturer)
	                $where .= ' OR (sec.id_manufacturer > 0 AND sec.id_manufacturer='.(int)$product->id_manufacturer.')';
	            ...
	            if($cates = $product->getCategories()) {
	                ...
	                $where .= ' OR (sec.id_category > 0 AND sec.id_category IN('.implode(',', $categories).'))';
	            }
                if ($product->id)
                    $where .= ' OR (sec.id_product LIKE "%,'.(int)$product->id.',%")';
                $where .= ')';
                break;
            case 28:
    $id_product = (int)Tools::getValue('id_product');
    $where .= ' AND sec.id_product LIKE "%,' . $id_product . ',%"';
    file_put_contents(_PS_ROOT_DIR_ . '/var/logs/steasycontent.log', "SQL WHERE para tipo 28: $where\n", FILE_APPEND);
    // Añadir log del resultado
    $sql = 'SELECT sec.*, secl.*, st_easy_content_shop.`id_shop` FROM `'._DB_PREFIX_.'st_easy_content` sec ... WHERE secl.`id_lang` = '.(int)$id_lang.' AND sec.`active`=1 '.$where;
    $result = Db::getInstance()->executeS($sql);
    file_put_contents(_PS_ROOT_DIR_ . '/var/logs/steasycontent.log', "Resultado para tipo 28: " . print_r($result, true) . "\n", FILE_APPEND);
    break;
            case 15:
            case 16:
                $where .= ' AND (sec.id_manufacturer = 0 AND sec.id_category = 0 OR (sec.id_manufacturer > 0 AND sec.id_manufacturer='.(int)Tools::getValue('id_manufacturer').'))';
                break;
            case 20:
            case 21:
                $where .= ' AND (sec.`id_st_blog` = '.(int)Tools::getValue('id_st_blog').' OR sec.`id_st_blog` = 0)';
                break;
            case 100:
                if (is_array($identify)) {
                    $identify = implode(',', $identify);
                }
                $where .= ' AND sec.`id_st_easy_content` IN ('.$identify.')';
                break;
            default:;
        }
        ...
        if ($total) {
            return Db::getInstance()->getvalue('
                ...
                ($active ? ' AND sec.`active`=1 ' : ' AND sec.`bo_tab`=0').
                $where
            );
        }
		...
		return Db::getInstance()->executeS('
            ...
            ($active ? ' AND sec.`active`=1 ' : ' AND sec.`bo_tab`=0').
            $where.'
            ORDER BY '.($active ? 'sec.`position`' : 'sec.`type`, sec.`id_cms`, sec.`position`').
            ($limit ? ' LIMIT '.$limit : ''));
...
file_put_contents(_PS_ROOT_DIR_ . '/var/logs/steasycontent.log', "Resultado para tipo $type: " . print_r($result, true) . "\n", FILE_APPEND);
return $result;
	}
    public static function getTabForAdminProduct($id_product=0)
    {
        Shop::addTableAssociation('st_easy_content', array('type' => 'shop'));
        return Db::getInstance()->executeS('
            SELECT secl.*
            ...
            '.Shop::addSqlAssociation('st_easy_content', 'sec').'
            LEFT JOIN `'._DB_PREFIX_.'st_easy_content_lang` secl ON (sec.`id_st_easy_content` = secl.`id_st_easy_content`)
            WHERE sec.`type` = 9 AND sec.`bo_tab` = 1 AND sec.`id_product`=",'.$id_product.',"');
    }
