...
class StEasyContent extends Module implements WidgetInterface
{
    ...
    public static $location = array(
        ...
        2800 => array('id' => 2800, 'name' => 'Right side of product price', 'hook' => 'DisplayProductPriceRight','column' => 1)
        );
    // key: type+00
    ...
    break;
			...
			if (!count($error) && $easy_content->validateFields(false) && $easy_content->validateFieldsLang(false))
            {
                ...
                {
                    $jon = trim(Tools::getValue('google_font_name'),'Â¤');
                    StEasyContentFontClass::deleteByContent($easy_content->id);
                    $jon_arr = array_unique(explode('Â¤', $jon));
        ...
        if(Validate::isLoadedObject($easy_content)){
            ...
            if(is_array($jon_arr) && count($jon_arr))
                foreach ($jon_arr as $key => $value) {
                    ...
                    $google_font_name .= $value['font_name'].'Â¤';
    ...
    'desc' => '<script type="text/javascript">
        $(document).ready(function(){
            $("input[name=\'type\']").on("change", function(){
                ...
                if ([6, 7, 8, 9, 10, 11, 26, 27, 28].indexOf(type) !== -1) {
                    $category_brand.show();
                    $products.show();
                    $category.show();
                    $manufacturer.show();
    ...
    public function hookDisplayProductPriceRight($params)
{
    ...
    $cache_id = '28-' . $id_product;
    if (!$this->isCached($this->templatePath . 'steasycontent.tpl', $this->stGetCacheId($cache_id))) {
        $this->_prepareHook(0, 28); // Tipo 28
    }
    return $this->fetch($this->templatePath . 'steasycontent.tpl', $this->stGetCacheId($cache_id));
}
    ...
    private function getApplyHook()
{
    ...
    // A«Ðadir tambi«±n los de location_extra si es necesario
    return $hooks;
}
